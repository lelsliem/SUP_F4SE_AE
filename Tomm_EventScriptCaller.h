...
template <> 
void tHashSet<VirtualMachine::IdentifierItem, UInt64>::Dump()
{
	class DumpVisitor : public VirtualMachine::IdentifierItem::IScriptVisitor
	{
	public:
		virtual bool Visit(VMIdentifier* obj)
		{
			if (!obj->m_typeInfo)
			{
				_DMESSAGE("Type info for VM identifier doesn't exist");
				return true;
			}
			string sScriptName = obj->m_typeInfo->m_typeName;
			boost::to_lower(sScriptName);
			//_MESSAGE("Found script>>%s, handle>>%x", sScriptName.c_str(), obj->GetHandle());
			//VMScripts.insert(std::make_pair(std::move(sScriptName), obj->GetHandle()));
...
namespace EventRunner {
	...
	void ProcessFile(string sFilePath)
	{
		...
		for (nlohmann::json::iterator it = j["Scripts"].begin(); it != j["Scripts"].end(); ++it) {
			...
			if (boost::iequals(sEvent, "OnMainGameloop")) {
			...
			boost::to_lower(NewEventRunner.s_ScriptName);
			NewEventRunner.s_FunctionName = Iter["FunctionName"].get<std::string>();
			boost::to_lower(NewEventRunner.s_FunctionName);
