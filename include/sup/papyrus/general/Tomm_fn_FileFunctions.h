...
UInt32 cmd_FileExists(StaticFunctionTag* base, BSFixedString name)
{
	...
	if (f_Does_FileExist(s_FileName))
	{
		...
		if (sup::string::icontains(s_FileName, "Data/"))
		{
				sup::string::replace_first(s_FileName, "Data/", "");
		...
		for (auto const& dir_entry : std::filesystem::directory_iterator{ s_FilePath })
		{
			if (iRequest)
			{
				if (iRequest == 1)
				{
					...
					{
						BSFixedString sTmp(dir_entry.path().filename().string().c_str());
						result.Push(&sTmp);
					}
				}
				else if (iRequest == 2)
...
VMArray<BSFixedString> cmd_FileGetFileTime(StaticFunctionTag* base, BSFixedString name, UInt32 iRequest)
{
	...
	BSFixedString sTmpY(tempchar);
	result.Push(&sTmpY); //Year
	StringCchPrintfA(tempchar, MAX_PATH, "%02d", stLocal.wMonth);
	BSFixedString sTmpM(tempchar);
	result.Push(&sTmpM); //Month
	StringCchPrintfA(tempchar, MAX_PATH, "%02d", stLocal.wDay);
	BSFixedString sTmpD(tempchar);
	result.Push(&sTmpD); //Day
	StringCchPrintfA(tempchar, MAX_PATH, "%02d", stLocal.wHour);
	BSFixedString sTmpH(tempchar);
	result.Push(&sTmpH); //Hour
	StringCchPrintfA(tempchar, MAX_PATH, "%02d", stLocal.wMinute);
	BSFixedString sTmpMin(tempchar);
	result.Push(&sTmpMin); //Minute
			...
			if (iRequest)
			{
				if (iRequest == 1)
				{
					if (dir_entry.is_directory())
					{
						...
						BSFixedString sTmp(TempPath.c_str());
					result.Push(&sTmp);
					}
