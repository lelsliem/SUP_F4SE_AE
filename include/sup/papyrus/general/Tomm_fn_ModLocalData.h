...
{
	// The dump is a diagnostic aid, so output goes to SUP_F4SE.log via _MESSAGE
	// (spdlog -> static CRT) rather than the game's in-console printf: the legacy
	// Console_Print path crashed the game's printf (ucrtbase %s scan, read @ 0x4)
	// on this exact data, and the log output is also far more useful for debugging.
	_MESSAGE("Dumping ModLocalData for>>>%s<<< STARTS", g_MLDArray[iModDataIndex]->ModName.c_str());
	_MESSAGE("Elements::");
	...
	for (auto it = g_MLDArray[iModDataIndex]->Elements.begin(); it != g_MLDArray[iModDataIndex]->Elements.end(); ++it)
	{
		...
		{
			_MESSAGE("--->>%s<<--, type is FLOAT, value is %f ", Iter->Name.c_str(), Iter->Val_float);
		}
		else if (Iter->Type == MLDElementType::kType_Int)
		{
			_MESSAGE("--->>%s<<--, type is INT, value is %d ", Iter->Name.c_str(), Iter->Val_int);
		}
		else if (Iter->Type == MLDElementType::kType_Ref)
		{
			if (f_IsFormValid(Iter->Val_Ref))
			{
				_MESSAGE("--->>%s<<--, type is REFERENCE, reference is %X ", Iter->Name.c_str(), Iter->Val_Ref->formID);
			}
			else {
				_MESSAGE("--->>%s<<--, type is REFERENCE, NOT VALID", Iter->Name.c_str());
			}
		}else if (Iter->Type == MLDElementType::kType_String)
		{
			// NB: %s takes a const char*, never a std::string object.
			_MESSAGE("--->>%s<<--, type is STRING, value is %s ", Iter->Name.c_str(), Iter->Val_String.c_str());
		}
		else if (Iter->Type == MLDElementType::kType_RefArray)
		{
			_MESSAGE("--->>%s<<--, type is Ref Array", Iter->Name.c_str());

			for (auto it2 = Iter->RefArray.begin(); it2 != Iter->RefArray.end();++it2)
			{
				_MESSAGE("Key is %d, reference is %X", static_cast<int>(it2 - Iter->RefArray.begin()), (*it2)->formID);
			}
		}
	}
	_MESSAGE("Dumping ModLocalData for>>>%s<<< ENDS", g_MLDArray[iModDataIndex]->ModName.c_str());
}
...
{
	string ModName = ModNameIN.c_str();
	string Key = KeyIN.c_str();
	int iModIndex = -1;
	int iCreated = 0;
