...
float cmd_ReadINIFloatFromFile(StaticFunctionTag* base, BSFixedString s_FilePath, BSFixedString s_IniSection, BSFixedString s_IniValue)
{
	...
	string s_FilePathINI = s_FilePath.c_str();
...
VMArray<BSFixedString> cmd_ReadINISectionKeysFromFile(StaticFunctionTag* base, BSFixedString s_FilePath, BSFixedString s_Section)
{
	...
	ini.GetAllKeys(s_Section.c_str(), keys);
...
UInt32 cmd_SetINIString(StaticFunctionTag* base, BSFixedString s_FilePath, BSFixedString s_IniSection, BSFixedString s_IniValue, BSFixedString Value)
{
	...
	ini.SetValue(s_IniSection.c_str(), s_IniValue.c_str(), Value.c_str());
	return ini.SaveFile(s_TempPath.c_str());
}
...
string f_GetMyDocumentsPath(UInt32 iRequest)
{
	...
	{
		char	path[MAX_PATH];
		HRESULT err = SHGetFolderPathA(NULL, CSIDL_MYDOCUMENTS | CSIDL_FLAG_CREATE, NULL, SHGFP_TYPE_CURRENT, path);
	...
	string sFinalPath = sDocPath.c_str();
...
GameSettingValue cmd_GetGameINISettingValue(StaticFunctionTag* base, BSFixedString s_SectionValue)
{
	...
	if (MySetting != NULL)
	{
		UInt32 SettingType = static_cast<UInt32>(MySetting->GetType());
		_DMESSAGE("Setting type>>>%d", SettingType);

		if (SettingType == static_cast<UInt32>(Setting::SETTING_TYPE::kInt) || SettingType == static_cast<UInt32>(Setting::SETTING_TYPE::kNone))
		{
			result.Set<bool>("Success", true);
			result.Set<float>("fValue", static_cast<float>(MySetting->GetUInt()));
		}
		else if (SettingType == static_cast<UInt32>(Setting::SETTING_TYPE::kFloat))
		{
			result.Set<bool>("Success", true);
			result.Set<float>("fValue", MySetting->GetFloat());
		}
		else if (SettingType == static_cast<UInt32>(Setting::SETTING_TYPE::kBinary))
		{
			result.Set<bool>("Success", true);
			result.Set<float>("fValue", MySetting->GetBinary() ? 1.0f : 0.0f);
		}
		else if (SettingType == static_cast<UInt32>(Setting::SETTING_TYPE::kString))
		{
			result.Set<bool>("Success", true);
			std::string sTmp(MySetting->GetString());
			BSFixedString TempString(sTmp.c_str());
			result.Set<BSFixedString>("sValue", sTmp.c_str());
		}
