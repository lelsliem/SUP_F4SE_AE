...
#include <stdio.h>
#include <windows.h>
// GdiplusPath.h uses unqualified `byte`, ambiguous between the SDK's global typedef and
// std::byte while `using namespace std;` (SUP_F4SE.cpp) / `using namespace RE;` are active.
// Pin `byte` to the SDK typedef for this include only.
#define byte ::byte
#include <gdiplus.h>
#undef byte
#include <time.h>
...
float cmd_CaptureScreenshotAlt(StaticFunctionTag* base, BSFixedString s_strFolderNameIN, BSFixedString s_strArgTempIN, float fPosXStart, float fPosXEnd, float fPosYStart, float fPosYEnd, float FormatIN, float iQuality) {
	...
	GdiplusStartup(&gdiplusToken, &gdiplusStartupInput, NULL);
	{
		...
		_DMESSAGE("IMAGE SAVED>>%ls", filename);
