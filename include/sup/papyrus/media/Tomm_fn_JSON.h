...
JSONOpenResult f_JSON_OpenFile(string sFilePath, UInt32 iCache, int iWriting)
{
	...
	sup::string::replace_all(sFilePath, "/", "\\");
...
{
	string sFilePathTemp = sFilePath.c_str();
	sup::string::to_lower(sFilePathTemp);
	sup::string::replace_all(sFilePathTemp, "/", "\\");
...
UInt32 cmd_JSONCacheFile(StaticFunctionTag* base, BSFixedString sFilePath, UInt32 iCreateNew)
{
	...
	sup::string::to_lower(s_TempPathRelative);
...
UInt32 cmd_JSONCloseFile(StaticFunctionTag* base, BSFixedString sFilePath, UInt32 iSave, BSFixedString sFilePathToSave)
{
	...
	string sFilePathToSaveTemp = sFilePathToSave.c_str();
...
{
	string s_LoweredKey = key.c_str();
	sup::string::to_lower(s_LoweredKey);

	string s_LoweredFilePath = sFilePath.c_str();
	sup::string::to_lower(s_LoweredFilePath);
