...
int f_BB_RefreshManageMenuMessage(UInt32 iButton, UInt32 iCurrentPage)
{
	...
	if (iCurrentPage == -1)
	{
		...
		{
			g_ManageMessages�urrentPage = -5;
		}
		//else if (iButton == 3) //Exit
		//{
		//	g_ManageMessages�urrentPage = -10;
	...
	if (g_ManageMessagesMaxMessages > 0)
	{
		...
		if (MessageTemp)
		{
			...
			g_ManageMessages�urrentPage = -20;
		}
	}
	else {
	...
	for (auto it = g_Activators.begin(); it != g_Activators.end(); it++)
	{
		...
		if (Iter->reference->formID == reference->formID)
		{
			...
			if (MessageTemp)
			{
				...
				_DMESSAGE("message text>>>%s", g_BBLastActivatedMessageString.c_str());
...
UInt32 cmd_BBPostMessage(StaticFunctionTag* base, UInt32 iMessageType, UInt32 iLaunchValue)
{
	...
	_DMESSAGE("SUP_BB::Message to send>>>>%s", jMessage.dump().c_str());
	...
	float fPosX = (*g_player)->GetPosition().x;
	float fPosY = (*g_player)->GetPosition().y;
	float fPosZ = (*g_player)->GetPosition().z;
	...
	//_DMESSAGE("3");
	const char* CellSourcePluginName = ChosenMod->filename;
...
void f_BB_SetMessageButtons(vector<string> vStrings)
{
	...
	BSSimpleList<MESSAGEBOX_BUTTON*>& ButtonList = g_TextInputMessage->buttonList;

	if (vStrings.size() == 0)
	{
		_DMESSAGE("No buttons to assign");
		return;
	}

	if (ButtonList.empty())
	{
		_DMESSAGE("MessageBox button list is empty");
		return;
	}

	ButtonList.front()->text = vStrings.at(0).c_str();
	...
	for (; it != ButtonList.end(); ++it)
	{
		...
		if (iTemp > vStrings.size())
		{
			...
			(*it)->text = vStrings.at(iButtonCount).c_str();
		}
	...
	for (auto it = g_BBAllWords.begin(); it != g_BBAllWords.end(); it++)
	{
		for (auto it2 = Iter->vWords.begin(); it2 != Iter->vWords.end(); it2++)
		{
			...
			for (auto it3 = Iter2->vWords.begin(); it3 != Iter2->vWords.end(); it3++)
			{
				...
				bool b_res = sup::string::icontains(Iter3.second.c_str(), g_CurrentFilterRequest.c_str());
...
void f_BB_AddChosenWordToCurrentMessage(BBWord* ChosenWord, int iRequest)
{
	...
	std::pair<int, string> TempPair = ChosenWord->vWords.at(iRequest);		_DMESSAGE("TempPair>>%s, key is >>%d ", TempPair.second.c_str(), TempPair.first);
...
int f_BB_ProcessWordByButtonPress(int iButton)
{
	...
	BBWordType* WordArray = f_BB_GetBBWordClassByKey(g_TextInput�urrentWordTypeKey);
	if (WordArray == NULL)
	{
		_DMESSAGE("Can't find words by request");
		return -1;
	...
	_DMESSAGE("g_TextInput�urrentWordTypeKey>>>%d", g_TextInput�urrentWordTypeKey);
	_DMESSAGE("g_TextInputOffsetPage>>>%d", g_TextInputOffsetPage);

	iButton = g_TextInput�urrentWordTypeKey + g_TextInputOffsetPage * 17  + iButton -3; // -3 because first 3 buttons are for navigating menu
...
{
	g_TextInput�urrentWordTypeKey = -1;
	g_CurrentFilterRequest.clear();
...
int f_BB_RefreshTextInputMessage(UInt32 iButton, UInt32 iCurrentPage)
{
	...
	if (iCurrentPage == -1)
	{
		...
		else if (iButton == g_TextInputCurrentPostMessageButtonNumber)
		{
			...
			for (auto it = g_BBAllWords.begin(); it != g_BBAllWords.end(); it++)
			{
				...
				{
					g_TextInput�urrentWordTypeKey = Iter->StartKey;
					g_TextInput�urrentPage = 100;
					_DMESSAGE("Found Iter menu button for %s", Iter->WordTypeName.c_str());
					if (iButton == -200) // Filling filtered words array
					{

						vStrings = f_BBGetFilteredStringsFromPool(g_TextInput�urrentWordTypeKey);
					}
					else {
						

						vStrings = f_BBGetStringsFromPool(g_TextInput�urrentWordTypeKey);
					}
	...
	if (g_TextInput�urrentWordTypeKey == t_BB_WordTypeStartKey_FilteredWords)
	{
	...
	if (g_TextInput�urrentPage == 100)
	{
		if (g_TextInput�urrentWordTypeKey == t_BB_WordTypeStartKeyMisc)
		{
	...
	else if (g_TextInput�urrentPage == -1)
	{
		s_FinalMessage += "\nTIP:You can search for words by typing on your keyboard.\n";
		s_FinalMessage += "Please choose what type of word you want to add: ";


	}else if (g_TextInput�urrentPage == 200)
	{
	...
	if (iKeyUP == 0)
	{
		...
		if (g_BBMenuStage == 100)
		{
			...
			if (g_CurrentFilterRequest.length() == 0)
			{
				...
				g_TextInput�urrentPage = -250; // for papyrus processing which will set this value to -1 and button to -200
				g_CurrentFilterRequest.clear();
			}
			else {
				g_TextInput�urrentPage = -200; // for papyrus processing which will set this value to -1 and button to -200
			}



			string sTemp = "MessageBoxMenu";
			BSFixedString bs_MenuName = sTemp.c_str();
			g_uiMessageManager->SendUIMessageImpl(bs_MenuName, kMessage_Refresh);
