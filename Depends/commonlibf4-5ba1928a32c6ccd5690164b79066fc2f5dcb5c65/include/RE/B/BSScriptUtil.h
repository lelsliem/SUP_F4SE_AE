...
namespace RE::BSScript
{
	namespace detail
	{
		...
		[[nodiscard]] consteval auto make_structure_tag(
			...
			-> REX::TStaticString<CharT, N1 + 1 + N2 + 1>
		{
	...
	public:
		// SUP fix: TStaticString stores N chars with NO trailing null, but BSFixedString's
		// string_view ctor requires the view to be null-terminated at data()[length()].
		// make_structure_tag now returns an N+1 TStaticString whose last char is '\0', so
		// the name below is length N but null-terminated (fixes "failed to get type info
		// for structure" for structs whose name length landed on non-null trailing bytes).
		static constexpr std::string_view name{ _full.data(), _full.length() - 1 };
	...
	[[nodiscard]] T UnpackVariable(const Variable& a_var)
	{
		...
		if (!in) {
			// A `None` array argument (e.g. `Var[] x = None`) arrives as an
			// Array-typed Variable holding a null array pointer. Treat it as an
			// empty array instead of dereferencing null (matches F4SE's VMArray).
			return T();
		}
