...
namespace RE::BSScript
{
	...
	[[nodiscard]] T UnpackVariable(const Variable& a_var)
	{
		...
		// A Variable can carry an array *type* with a null backing Array (scripts pass a
		// None/uninitialized value where an array is expected, or a degenerate Var[] element).
		// Iterating `in->elements` would deref [Array+0x18] on null and CTD (seen on NISTRON
		// device reset -> SUP.SetEventHandler with a null-backed filters array).
		if (!in) {
			return T();
		}
