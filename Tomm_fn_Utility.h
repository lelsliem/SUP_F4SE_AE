...
BSFixedString cmd_ClipboardToString(StaticFunctionTag* base)
{
	...
	_DMESSAGE("ClipboardToString result is %s", sResult);
...
UserTimeStruct cmd_GetUserTimeStruct(StaticFunctionTag* base)
{
	UserTimeStruct result;
	const auto now = std::chrono::system_clock::now();
	const auto in_time_t = std::chrono::system_clock::to_time_t(now);
	struct tm time_info;
	const auto errno_value = localtime_s(&time_info, &in_time_t);
	...
	UInt32 iTempValue = time_info.tm_year + 1900;
	result.Set<UInt32>("Year", iTempValue);
	iTempValue = time_info.tm_mon + 1;
	result.Set<UInt32>("Month", iTempValue);
	iTempValue = time_info.tm_mday;
	result.Set<UInt32>("Day", iTempValue);
	iTempValue = time_info.tm_hour;
	result.Set<UInt32>("Hour", iTempValue);
	iTempValue = time_info.tm_min;
	result.Set<UInt32>("Minute", iTempValue);
	iTempValue = time_info.tm_sec;
	result.Set<UInt32>("Second", iTempValue);
