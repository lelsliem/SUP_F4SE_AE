...
// Helper function to get power grid for a target (replaces Workshop::ExtraData::GetPowerGridForTarget)
inline PowerUtils::PowerGrid* GetPowerGridForTarget(Workshop::ExtraData* extraData, TESObjectREFR* target)
{
    // Stub implementation - would need Address Library function pointer for full implementation
...
// Global stub function for getting object handles for events
// Needs full implementation for script event handling
struct ObjectHandleResult {
	UInt64 handle;
	bool bSuccess;
	ObjectHandleResult() : handle(0), bSuccess(false) {}
};

inline ObjectHandleResult GetObjectHandleForEvent(SUPEvent* CurrentEvent)
{
	ObjectHandleResult result;
	// Stub implementation - script event handling needs Address Library integration
	return result;
}
...
public:
	...
	// ObjectHandleResult and GetObjectHandleForEvent moved to global scope above
	...
	void CheckAndStoreFunctions() // No need to lock, happens at the start of the game
	{
		for (auto it = v_EventList.begin(); it != v_EventList.end();)
		{
			...
			if (!result.bSuccess)
			{
				_MESSAGE("Cannot find Script>>> %s in the VM list - removing from SUP Script runner.", Iter->s_ScriptName);
				delete Iter;
				it = v_EventList.erase(it);
				continue;
			}
	...
	void fn_OnPowerConnectionEvent_SendEvent(bool bAdded, TESObjectREFR* workshopRef, TESObjectREFR* akRef1, TESObjectREFR* akRef2, TESObjectREFR* akWireRef, Workshop::ExtraData* WorkshopExtraData = NULL)
	{
		...
		if (akRef1)
		{
			auto pGrid1 = GetPowerGridForTarget(WorkshopExtraData, akRef1);

			if (pGrid1)
			{
				for (UInt32 i = 0; i < WorkshopExtraData->powerGrid.count; i++)
				{
					PowerUtils::PowerGrid* pGrid;
					WorkshopExtraData->powerGrid.GetNthItem(i, pGrid);
		...
		if (akRef2)
		{
			auto pGrid2 = GetPowerGridForTarget(WorkshopExtraData, akRef2);
	...
	void ProcessEventOnPlayerMapMarkerStateChangeEvent(bool bAdded,  NiPoint3 coords, TESWorldSpace* TargetWorldspace = NULL)
	{
		...
		if (HasEvents())
		{
			...
			if (!bAdded)
			{
				...
				if (MarkerRef)
				{
					coords.x = MarkerRef->pos.x;
					coords.y= MarkerRef->pos.y;
					coords.z = MarkerRef->pos.z;
	...
	void ProcessEventOnRagdollStart(Actor* ActorRef, NiPoint3* PosInfo, float force)
	{
		...
		if (HasEvents())
		{
			...
			SUPEvent::SUPEventFilters::SUPSmartFilter NewFilter2(SUPEvent::SUPEventFilters::k_FilterSecond, SUPEvent::SUPEventFilters::k_FilterFormID);
			std::get<vector<UInt32>>(NewFilter.List).push_back(ActorRef->baseForm->formID);
	...
	void ProcessEventOnCrosshairRefChange(bool OnCrosshairOn,TESObjectREFR* NewCrosshairRef, UInt32 CrosshairType)
	{
		...
		if (NewCrosshairRef->baseForm)
		{
			std::get<vector<UInt32>>(NewFilter3.List).push_back(NewCrosshairRef->baseForm->formID);
			std::get<vector<float>>(NewFilter4.List).push_back(NewCrosshairRef->baseForm->formType);
		}
		else {
...
namespace SUPEventFilterHelper {
	...
	void GetFormIDFromVMValue(vector<UInt32>& vList, VMValue& mValue, VMVariable& var, int& iFilterInvalidNum)
	{
		...
		{
			//_DMESSAGE(" CURRENT VAR IS REFR - pushing");
			vList.push_back(mValue.data.id->GetHandle());
			return;
		}
		...
		{
			_DMESSAGE(" CURRENT VAR IS FORM- pushing");
			vList.push_back(mValue.data.id->GetHandle());
			return;
		}
		...
		if (iAcceptedType == SUPEvent::SUPEventFilters::k_FilterString)
		{
			...
			if (type == VMValue::kType_String)
			{
				...
				if (var.Get(&MyString))
				{
					...
					string sLowerString = MyString;
					boost::to_lower(sLowerString);
