...
VMArray<TESObjectREFR*> cmd_GetPowerGridElements(StaticFunctionTag* base, TESObjectREFR* WorkshopRef, UInt32 iGridID)
{
	...
	PowerUtils::PowerGrid* pGrid = NULL;
	WorkshopData->powerGrid.GetNthItem(iGridID, pGrid);
	if (pGrid)
	{
...
bool cmd_AreDirectlyConnected(StaticFunctionTag* base, TESObjectREFR* WorkshopRef, TESObjectREFR* TempRefA, TESObjectREFR* TempRefB)
{
	...
	return WorkshopCompat::AreDirectlyConnected(WorkshopData, TempRefA, TempRefB);
}
...
UInt32 cmd_RemovePowerGrid(StaticFunctionTag* base, TESObjectREFR* WorkshopRef, UInt32 iIndexToRemove)
{
	...
	if (iIndexToRemove > WorkshopData->powerGrid.count - 1)
	{
		_DMESSAGE("Index out of range");
		return -1;
	}

	if (WorkshopData->powerGrid.Remove(iIndexToRemove))
	{
		return 1;
	}
	else {
		...
		if (pGrid)
		{
			...
			for (auto it = pGrid->adjacencyMap.begin(); it != pGrid->adjacencyMap.end(); it++)
			{
				...
				for (auto it2 = Iter.second->begin(); it2 != Iter.second->end(); it2++)
				{
					...
					//	//if (connector->baseForm)
					//	//{
					//	//	_DMESSAGE("Connector base form>>%x", connector->baseForm->formID);
					//	//}
					//	//else {
...
VMArray<TESObjectREFR*>  cmd_ScanPowerGridForFaults(StaticFunctionTag* base, TESObjectREFR* WorkshopRef, UInt32 iPowerGridID)
{
	...
	PowerUtils::PowerGrid* pGrid = NULL;
	WorkshopData->powerGrid.GetNthItem(iPowerGridID, pGrid);


	if (!pGrid)
	{
		_DMESSAGE("Cannot extract power grid by specified id, count>>%d", WorkshopData->powerGrid.count);
		return result;
	}
...
bool cmd_WorkshopCalculateCapacityAndLoad(StaticFunctionTag* base, TESObjectREFR* WorkshopRef)
{
	...
	_DMESSAGE("ALL valid - calling");
	WorkshopCompat::CalculateCapacityAndLoad(WorkshopData);
	return true;
}
...
WorkshopStatus GetWorkShopStatus(Workshop::ExtraData* WorkshopData, bool bGetWorkshopStatus, bool bGetPoweredItems)
{
	...
	{
		result.PowerGrids.reserve(WorkshopData->powerGrid.count);
	...
	for (UInt64 i = 0; i < WorkshopData->powerGrid.count; i++)
	{
		...
		if (bGetWorkshopStatus)
		{
			...
			NewPowerGrid.iPoweredItemsCount = pGrid->currentlyPowered.count;
			NewPowerGrid.Pointer = pGrid;
			NewPowerGrid.ID = i;
			//_DMESSAGE("PowerGridBefore,ID>>%d,capacity>>%f,load>>%f", i, pGrid->capacity, pGrid->load);
		...
		if (bGetPoweredItems)
		{
			result.PowerGrids.reserve(result.PowerGrids.capacity() + pGrid->currentlyPowered.count);
...
PowerGridChanges HasWorkshopChanged(WorkshopStatus& WorkshopStatusBefore, Workshop::ExtraData* WorkshopData) {
	...
	if (WorkshopData->powerGrid.count != WorkshopStatusBefore.PowerGrids.size())
	{
	...
	for (UInt64 i = 0; i < WorkshopData->powerGrid.count; i++)
	{
		...
		for (auto it = WorkshopStatusBefore.PowerGrids.begin(); it != WorkshopStatusBefore.PowerGrids.end(); it++)
		{
			if (Iter.Pointer == pGrid)
			{
				...
				if (pGrid->currentlyPowered.count != Iter.iPoweredItemsCount)
				{
					result.AffectedPowerGrids.push_back(Iter.ID);
					WorkshopStatusBefore.PowerGrids.erase(it);
					result.bChanged = true;
		...
		for (UInt32 i = 0; i < LinkedRefsExtra->LinkedChildren.count; i++)
		{
			...
			if (pLink.keyword == g_SUPWorkshopKeyword)
			{
				...
				if (CurrentRef && RadiatorRef->formID != CurrentRef->formID)
				{
					...
					if (WorkshopCompat::IsPowered(WorkshopData, CurrentRef))
					{
...
void RadiatorFixProcessPowerOnOffEvent(bool bPoweredON, TESObjectREFR* Radiator, TESObjectREFR* WorkshopRef, Workshop::ExtraData* WorkshopData)
{
	...
	float fRadiationValue = Radiator->actorValueOwner.GetValue(g_RadiationActorValue);
	...
	if (bPoweredON)
	{
		...
		//if (!WorkshopData->IsPowered(Radiator))
		//{
		...
		for (auto it = ItemsToConnect.begin(); it != ItemsToConnect.end(); it++)
		{
			...
			//WorkshopData->AddConnection(refA, refB, wireRef);
		...
		for (auto it = WirelessConnections.begin(); it != WirelessConnections.end(); it++)
		{
			...
			for (auto it3 = ConnectPoints.begin(); it3 != ConnectPoints.end(); it3++)
			{
				...
				{
					_DMESSAGE("NISTON::Snapped connection>>%x   %s  found for radiator - NOT removing it.", Iter3->formID, Iter3->GetFullName());
					bFound = true;
					break;
				}
			...
			if (!bFound)
			{
				...
				WorkshopCompat::RemoveConnection(WorkshopData,Iter.Connection1, Iter.Connection2, (TESObjectREFR*)NULL, false);
				_DMESSAGE("NISTON::Removing wireless connection, Radiator>>%x %s,Receiver>>%x %s", Iter.Connection1->formID, Iter.Connection1->GetFullName(), Iter.Connection2->formID, Iter.Connection2->GetFullName());
			}
...
void WirelessFixProcessChanges(WorkshopStatus& WorkshopStatusBefore, Workshop::ExtraData* WorkshopData, TESObjectREFR* WorkshopRef, UInt32 SelfFormID, int &iCheckForSelf, bool OnlyRemove)
{
	...
	for (UInt64 i = 0; i < WorkshopData->powerGrid.count; i++)
	{
		...
		for (UInt64 i = 0; i < pGrid->currentlyPowered.count; i++)
		{
			...
			//NewPoweredItems.push_back(CurrentlyPoweredID);
			bool bFound = false;
			...
			{
				// Item switched ON.
				//Console_Print("Item switched ON>>%x", CurrentlyPoweredID);
	...
	{
		// Item switched OFF
		//Console_Print("Item switched OFF>>%x", Iter);
		...
		if (CurrentObject)
		{
			...
			//_DMESSAGE("ITEM INVALID");
			//Console_Print("ITEM INVALID");
		}
	}
...
bool cmd_RemoveGridConnection(StaticFunctionTag* base, TESObjectREFR* WorkshopRef, TESObjectREFR* TempRefA, TESObjectREFR* TempRefB, TESObjectREFR* refSpline)
{
	...
	WorkshopCompat::RemoveConnection(WorkshopData,TempRefA, TempRefB, refSpline, false);
...
bool cmd_AddGridConnection(StaticFunctionTag* base, TESObjectREFR* WorkshopRef, TESObjectREFR* TempRefA, TESObjectREFR* TempRefB, TESObjectREFR* refSpline)
{
	...
	WorkshopData->AddConnection(TempRefA, TempRefB, refSpline);
...
bool cmd_WorkshopCleanUpEmptyPowerGrids(StaticFunctionTag* base, TESObjectREFR* WorkshopRef)
{
	...
	_DMESSAGE("ALL valid - calling");
	WorkshopCompat::CleanUpEmptyPowerGrids(WorkshopData);
	return true;
}
...
bool cmd_ReestablishTerminalLinks(StaticFunctionTag* tag, TESObjectREFR* REF)
{
	...
	if (REF->baseForm && REF->baseForm->formType != kFormType_TERM)
	{
...
//		}
//
//		if (pGrid->currentlyPowered.count != WorkshopStatusBefore.PowerGrids.at(i).iPoweredItemsCount)
//		{
//			_DMESSAGE("Power grid#%d has currentlyPowered count changed, old load>>%d, new load>>%d", i, WorkshopStatusBefore.PowerGrids.at(i).iPoweredItemsCount, pGrid->currentlyPowered.count);
//			return 5;
//		}
//
//	}
