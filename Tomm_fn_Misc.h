...
bool cmd_DrawSpline(StaticFunctionTag* base, TESObjectREFR* refA, TESObjectREFR* refB, TESObjectREFR* splineRef, float Thickness, float Slack)
{
   ...
   _DMESSAGE("DrawLineStarts");
    ...
    if (!splineRef || !refA || !refB || refA == refB || !refA->GetObjectRootNode() || !refB->GetObjectRootNode()) {
    ...
    TESWorldSpace* worldspace = CALL_MEMBER_FN(splineRef, GetWorldspace)();

    NiPoint3 rot;
    MoveRefrToPosition(splineRef, &nullHandle, parentCell, worldspace, &refA->pos, &rot);

    Workshop::ContextData contextData(*g_player);

    SplineUtils::UpdateSpline(&contextData, splineRef, refB, 0, refA, 0);
    SplineUtils::ConnectSpline(refA, 0, refB, 0, splineRef);
    ...
    if (splineParams) {
        splineParams->thickness = Thickness;
        splineParams->unk18 = Slack;
...
TESObjectREFR* cmd_GetWorldLocationMarkerRef(StaticFunctionTag* base, BGSLocation* Location)
{
    ...
    LookupREFRByHandle(Location->unk70, refr);
...
VMArray <SUPFurnitureMarkerData> cmd_GetFurnitureMarkersFromFurniture(StaticFunctionTag*, TESForm* thisForm)
{
    ...
    for (UInt64 i = 0; i < pFurnitureSUP->markersArray.count; i++)
    {
        ...
        ResultElement.Set<TESForm*>("Keyword", MarkerInst.Keyword);
...
bool cmd_SetGlobalTimeMultiplier(StaticFunctionTag* base, float fTimeMult)
{
    ...
    string sCommand = "sgtm " + to_string(fTimeMult);
    ...
    root->CreateString(&args, sCommand.c_str());

    GFxValue value;
    root->Invoke("root1.AnimHolder_mc.Menu_mc.BGSCodeObj.executeCommand", &value, &args, 1);
    _DMESSAGE("Invoked");*/
    return true;
}
...
{
    string sMyScriptName = ScriptName;
    boost::to_lower(sMyScriptName);

    return AttachedScriptReader::IsScriptLoaded(sMyScriptName.c_str());
}
...
bool cmd_IsScriptLoadedForForm(StaticFunctionTag* base, BSFixedString ScriptName, TESForm* MyForm)
{
    ...
    return AttachedScriptReader::IsScriptLoadedForForm(sMyScriptName.c_str(), MyForm);
}
...
{
    return vCurrentRadioFrequencies;
}
...
bool cmd_SetChargenPresetFlag(StaticFunctionTag* tag, TESForm* thisForm, bool bTrue)
{
    ...
    _DMESSAGE("Flags before are>>%d", pTESActorData->flags);

    if (bTrue)
    {
        pTESActorData->flags = pTESActorData->flags | TESActorBaseData::kFlagIsPreset;
    }
    else {
        pTESActorData->flags &= ~(TESActorBaseData::kFlagIsPreset);
    }

    _DMESSAGE("Flags after are>>%d", pTESActorData->flags);
...
VMArray<TESForm*> cmd_GetLoadedFormsByType(StaticFunctionTag* tag, UInt32 iType, BSFixedString PluginName)
{
    ...
    tArray<TESForm*>* MyArr = (tArray<TESForm*>*)(Adress + iType);
...
BSFixedString cmd_GetAmmoShellCasingModelPath(StaticFunctionTag* tag, TESAmmo* MyAmmo)
{
    ...
    return MyAmmo->shellCasing.GetModelName();
}
...
bool cmd_SetAmmoShellCasingModelPath(StaticFunctionTag* tag, TESAmmo* MyAmmo, BSFixedString NewPath)
{
    ...
    MyAmmo->shellCasing.SetModelName(NewPath);
}
...
bool cmd_FailQuest(StaticFunctionTag* tag, TESQuest* MyQuest)
{
    ...
    // TESQuest::FailQuest not available in F4SE 0.7.7
    // Setting quest failed flag directly
    _DMESSAGE("FailQuest: Function not available in F4SE 0.7.7");
    return false;
...
TESForm* CheckForRefAndGetForm(TESForm* CurrentForm)
{
    ...
    TESObjectREFR* CurrentRef = (TESObjectREFR*)Runtime_DynamicCast(CurrentForm, RTTI_TESObjectREFR, RTTI_TESForm);

    if (CurrentRef)
    {
        if (CurrentRef->baseForm)
        {
            return CurrentRef->baseForm;
        }
        else {
            return NULL;
        }
...
BSFixedString cmd_GetOverridingMod(StaticFunctionTag* tag, TESForm* CurrentForm)
{
    ...
    if (!CurrentForm->unk08->size)
    {
        _DMESSAGE("Form is not overriden by anything.")
        return "";
    }
    int iPos = CurrentForm->unk08->size - 1;
    return CurrentForm->unk08->entries[iPos]->name;
}
...
VMArray<BSFixedString> cmd_GetOverridingMods(StaticFunctionTag* tag, TESForm* CurrentForm)
{
    ...
    for (int i = 0; i < CurrentForm->unk08->size; i++)
    {
        result.Push(&BSFixedString(CurrentForm->unk08->entries[i]->name));
    }
...
bool cmd_IsOverridingMod(StaticFunctionTag* tag, TESForm* CurrentForm, BSFixedString ModName)
{
    ...
    int iPos = CurrentForm->unk08->size - 1;
    return boost::iequals(ModName.c_str(), CurrentForm->unk08->entries[iPos]->name);
}
...
VMArray<TESForm*> cmd_GetInventoryItemsByType(StaticFunctionTag* tag, TESObjectREFR* ObjectRef, UInt32 type, bool bReturnMultiple)
{
    ...
    if (!ObjectRef->inventoryList->items.count)
    {
        _DMESSAGE("Inventory list count is 0");
        return result;
    }

    ObjectRef->inventoryList->inventoryLock.LockForRead();


    UInt32 iPos = 0;

    while (iPos < ObjectRef->inventoryList->items.count)
    {
        TESForm* CurrentForm = ObjectRef->inventoryList->items[iPos].form;

        if (CurrentForm && CurrentForm->formType == type)
        {
            if (bReturnMultiple)
            {
                BGSInventoryItem::Stack* CurrentStack = ObjectRef->inventoryList->items[iPos].stack;
                if (CurrentStack)
                {
    ...
    ObjectRef->inventoryList->inventoryLock.UnlockRead();
...
VMArray<TESForm*> cmd_GetInventoryItemsByTypes(StaticFunctionTag* tag, TESObjectREFR* ObjectRef, VMArray<UInt32> types, bool bReturnMultiple)
{
    ...
    while (iPos < ObjectRef->inventoryList->items.count)
    {
        ...
        {
            if (CurrentForm && CurrentForm->formType == Iter)
            {
                if (bReturnMultiple)
                {
                    BGSInventoryItem::Stack* CurrentStack = ObjectRef->inventoryList->items[iPos].stack;
...
BGSKeyword* cmd_GetKeywordByName(StaticFunctionTag* tag, BSFixedString sKeywordName)
{
    ...
    tArray<TESForm*>* Adress = std::addressof((*g_dataHandler)->arrNONE);
    tArray<TESForm*>* MyArr = (tArray<TESForm*>*)(Adress + kFormType_KYWD);

    for (int i = 0; i < (*MyArr).count; i++)
    {
        TESForm* CurrentObject = NULL;
        (*MyArr).GetNthItem(i, CurrentObject);
        BGSKeyword* CurrentKeyword = (BGSKeyword*)CurrentObject;

        if (CurrentKeyword->keyword == sKeywordName)
        {
...
ActorValueInfo* cmd_GetActorValueByName(StaticFunctionTag* tag, BSFixedString sKeywordName)
{
    ...
    tArray<TESForm*>* Adress = std::addressof((*g_dataHandler)->arrNONE);
    tArray<TESForm*>* MyArr = (tArray<TESForm*>*)(Adress + kFormType_AVIF);

    for (int i = 0; i < (*MyArr).count; i++)
    {
        TESForm* CurrentObject = NULL;
        (*MyArr).GetNthItem(i, CurrentObject);
        ActorValueInfo* CurrentAV = (ActorValueInfo*)CurrentObject;

        if (_stricmp(CurrentAV->avName, sKeywordName.c_str()) == 0)
        {
            return CurrentAV;
        }
...
//}
//else {
//    std::string sTemp = CurrentScriptName;
//}
