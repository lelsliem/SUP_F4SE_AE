...
// F4SE 0.7.7 compatibility: CellCoord no longer exists in TESObjectCELL
// For exterior cells, coordinates are stored in the worldspace cell map key
struct CellCoordCompat {
    SInt32 X;
    SInt32 Y;
};

// Get cell coordinates from worldspace cell map
// For exterior cells, the key encodes X in high 16 bits, Y in low 16 bits
inline CellCoordCompat GetCellCoordinates(TESObjectCELL* cell) {
    CellCoordCompat result = {0, 0};
    if (!cell || !cell->worldSpace) return result;

    // Search for cell in worldspace CellMap to get coordinates from key
    for (auto it = cell->worldSpace->CellMap.begin(); it != cell->worldSpace->CellMap.end(); ++it) {
        if (it->second == cell) {
            // Key format: X in high 16 bits, Y in low 16 bits
            result.X = static_cast<SInt32>(it->first >> 16);
            result.Y = static_cast<SInt16>(it->first & 0xFFFF);
            return result;
        }
    }
    return result;
...
// Helper macro to emulate old CellCoord->X/Y access pattern
#define GET_CELL_X(cell) (GetCellCoordinates(cell).X)
#define GET_CELL_Y(cell) (GetCellCoordinates(cell).Y)
...
int g_TextInputCurrentPage = 0;
int g_TextInputCurrentWordTypeKey = 0;
...
int g_ManageMessagesCurrentPage = 0;
		...
		if (Iter->iDontInclude == false)
		{
			...
			if (Iter->iType != t_BB_WordTypeClassMisc)
			{
				std::sort(Iter->vWords.begin(), Iter->vWords.end(), [](BBWord* a, BBWord* b) -> bool
					{
						return boost::to_lower_copy(a->vWords.at(0).second) < boost::to_lower_copy(b->vWords.at(0).second);
					});
...
TESObjectREFR* f_BB_GetReferenceFromMod(UInt32 mod_Idx, UInt32 form_Dec)
{
	...
	//if (form_Result->baseForm)
	//{
	//	if (form_Result->baseForm->formType != kFormType_STAT)
	//	{
	//		_DMESSAGE("SUP_BB::Reference BASE FORM has different type >>>>%d", form_Result->baseForm->formID);
	//		return NULL;
	//	}
...
UInt32 f_BB_GetExteriorCellFromOnlineTable(string CellPluginName, int CellFormID)
{
	...
	UInt32 mod_Idx = (*g_dataHandler)->GetLoadedModIndex(CellPluginName.c_str());
...
TESObjectCELL* f_BB_GetInteriorCellFromOnlineTable(string PluginName, int CellFormID)
{
	...
	UInt32 mod_Idx = (*g_dataHandler)->GetLoadedModIndex(PluginName.c_str());
...
bool f_BB_StoreMarkersFromCell()
{
	...
	if (MyCell)
	{
		...
		for (UInt64 i = 0; i < MyCell->objectList.count; i++)
		{
			MyCell->objectList.GetNthItem(i, TempRef);
			//_DMESSAGE("Type is>>%d", TempRef->baseForm->formType);
			if (TempRef && TempRef->baseForm && TempRef->baseForm->formType == kFormType_ACTI)//kFormType_ACTI)//kFormType_NPC_)// )
			{
		...
		//if (pRefr && pRefr->baseForm->typeID == kFormType_TESCreature)//kFormType_TESObjectLIGH)//) //kFormType_TESObjectACTI) //)kFormType_BGSNote 
		//
...
{
	const ModInfo* FalloutESM = (*g_dataHandler)->LookupModByName("FalloutNV.esm");

	if (FalloutESM)
	{
		return FalloutESM->modIndex;
	}
	else
	{
		return -1;
...
{
	UInt8 mod_Idx = (*g_dataHandler)->GetLoadedModIndex(s_BBPluginName.c_str());
...
void f_BB_OnDeterredInit()
{
	if (f_BB_IsPluginLoaded())
	{
		...
		CALL_MEMBER_FN(g_BBLoadFunctionName, Set)(ptr2);

		g_BBMoveToFunctionName = new BSFixedString;
		const char* ptr3 = "MoveMessageToPlayer"; //Notification
		CALL_MEMBER_FN(g_BBMoveToFunctionName, Set)(ptr3);
...
TESObjectREFR* f_BB_GetRefFromMod(string sPluginFileName, string s_ScriptFormID)
{
	...
	UInt32 mod_Idx = (*g_dataHandler)->GetLoadedModIndex(sPluginFileName.c_str());
...
void f_BB_SetMessageBodyText(string sText, int iRequest)
{
	...
	if (iRequest == 1)
	{
		pFullName = DYNAMIC_CAST(g_MessageString1Ref->baseForm, TESForm, TESFullName);

	}
	else if (iRequest == 2)
	{
		pFullName = DYNAMIC_CAST(g_MessageString2Ref->baseForm, TESForm, TESFullName);
	}
	else if (iRequest == 3)
	{
		pFullName = DYNAMIC_CAST(g_MessageString3Ref->baseForm, TESForm, TESFullName);
	}

	if (pFullName)
	{
		pFullName->name = sText.c_str();
	}
...
{
	for (auto it = worldspace->CellMap.begin(); it != worldspace->CellMap.end(); ++it) {
		CellCoordCompat coords = GetCellCoordinates(Iter.second);
		if (coords.X == iOffsetX && coords.Y == iOffsetY)
		{
			_DMESSAGE("CELL FOUND IN CELL MAP>>>%x", Iter.second->formID);
			return Iter.second;
		}
...
struct CellScanInfo // ripped off from original NVSE.
{
		...
		: curCell(NULL), cell(_cell), world(NULL), curX(0), curY(0), cellDepth(_cellDepth)
	{
		...
		{
			curX = cellCoords.X - cellDepth;
			curY = cellCoords.Y - cellDepth;
	...
	bool NextCell()		//advance to next cell in area
	{
		...
		do
		{
			if (curX - cellCoords.X == cellDepth)
			{
				if (curY - cellCoords.Y == cellDepth)
				{
					curCell = NULL;
					return false;
				}
...
vector<UInt32> f_BB_GetNearCells(TESObjectCELL* cell)
{
	...
	CellCoordCompat coords = GetCellCoordinates(cell);
	SInt8	OrigX = coords.X;
	SInt8	OrigY = coords.Y;
...
string f_BB_GetStringFromMessageArray(vector<int> v_Message)
{
	...
	{
		iFoundLetter = 0;
		_DMESSAGE("Processing#>>>%s", to_string(Iter));
		for (auto it2 = g_BBAllWords.begin(); it2 != g_BBAllWords.end(); it2++)
		{
			if (iFoundLetter)
				break;
...
{
	_DMESSAGE("f_BB_Refresh_MessageMenu() started");
	...
	GFxValue fxValueString;
	BSFixedString BS_MESSAGE(s_Final_Message.c_str());
	fxValueString.SetString(BS_MESSAGE);
	root->SetVariable("_root.Menu_mc.bodyText", &fxValueString);

	_DMESSAGE("f_BB_Refresh_MessageMenu() ended");
}
